...
// ---- Mob pathfinding AI ----
group PP_Mobs = Jass::CreateGroup();
...
void PP_OnEnterPortal() {
    ...
    if (Jass::IsUnitInGroup(u, PP_Mobs)) {
...
void PP_AddMob(unit u) {
    Jass::GroupAddUnit(PP_Mobs, u);
}
...
void PP_MobTimerCallback() {
    // Удалить мёртвых
    int count = Jass::GroupGetCount(PP_Mobs);
    group tempGroup = Jass::CreateGroup();
    for (int i = 0; i < count; i++) {
        unit u = Jass::GroupGetUnitByIndex(PP_Mobs, i);
        if (!Jass::IsUnitAlive(u)) {
            int uid = Jass::GetHandleId(u);
            Jass::FlushChildHashtable(PP_HT, uid);
            Jass::GroupAddUnit(tempGroup, u); // для безопасного удаления после цикла
        }
    }
    Jass::GroupRemoveGroupEx(PP_Mobs, tempGroup);
    Jass::DestroyGroup(tempGroup);
    // Обработать каждого моба
    count = Jass::GroupGetCount(PP_Mobs);
    for (int i = 0; i < count; i++) {
        unit u = Jass::GroupGetUnitByIndex(PP_Mobs, i);
        PP_ProcessMob(u);
    }
